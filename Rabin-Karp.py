...
def rool_hash(old_val, out_digit, in_digit, last_pos):
    ...
    for i in range(len_s - len_t + 1):
        if hash_s == hash_t:
            if matches(s, t, i, 0, len_t):
                return i
        if i < len_s - len_t:
            hash_s = rool_hash(hash_s, ord(s[i]), ord(s[i+len_t]), last_pos)
    return -1



s = "ADEADBCABABCABCDABDADBEAABCDABD"
t = "ABCDABD"


print(rabin_karp_matching(s, t))